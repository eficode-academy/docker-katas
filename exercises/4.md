...
* `EXPOSE` creates a hint for users of an image which provide services on ports. It is included in the information which
...
If you want to concatenate two layers (eg. the update and install [which is a good idea](https://docs.docker.com/engine/userguide/eng-image/dockerfile_best-practices/#run)), then do them in the same RUN command:
